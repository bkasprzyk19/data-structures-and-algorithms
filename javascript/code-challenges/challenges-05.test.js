...
const toLastNames = people => {
  let fullNames = people.map( (val,idx) => val.firstName + ' ' + val.lastName);
  return fullNames;
};
...
const addPurchases = (arr) => {
  let cart = arr.map( (val,idx) => val.purchasePrice);
  
  let sumCart = cart.reduce( function(accumulator,value,idx){
    accumulator = accumulator + value;
    return accumulator;
  }, 0);
  return sumCart;
